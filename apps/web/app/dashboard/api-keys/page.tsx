    ...
    <Chrome auth={auth}>
      ...
      <div className="mt-6 overflow-x-auto rounded-xl border border-slate-200 bg-white shadow-sm">
        <table className="w-full text-left text-sm">
          ...
          <tbody>
            {rows.length === 0 && <tr><td colSpan={5}><EmptyState title="No API keys yet." hint="Create one above to let a partner system authenticate and call the API." /></td></tr>}
            {rows.map((k) => (
              <tr key={k.id} className="border-b border-slate-50">
                ...
                <td className="px-5 py-3">
                  <StatusPill status={k.status.toLowerCase()} tone={k.status === 'ACTIVE' ? 'green' : 'gray'} />
                </td>
